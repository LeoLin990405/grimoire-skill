...
if [[ "$INPUT" =~ ^https?:// ]]; then
    ...
    # 2026-04-27: API 改为 file_urls[0] 直接是字符串（非对象 .url），向后兼容两种格式
    upload_url=$(echo "$resp" | jq -r 'if (.data.file_urls[0] | type) == "string" then .data.file_urls[0] else .data.file_urls[0].url end')
    batch_id=$(echo "$resp" | jq -r '.data.batch_id')

    # Upload file (2026-04-27: OSS pre-signed URL 签名不含 Content-Type，
    # 用 -T 而非 --data-binary，并显式清空 Content-Type 防 curl 默认加上 form-urlencoded)
    curl -s -X PUT "$upload_url" -H "Content-Type:" -T "$INPUT"
